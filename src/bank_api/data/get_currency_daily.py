...
def get_daily_currency_rates(currencies=['USD', 'EUR', 'GBP', 'TRY']):
    ...
    tbc_rates = get_tbc_exchange_rates(currencies)
    all_rates.extend(tbc_rates)

    if all_rates:
        df = pd.DataFrame(all_rates)
        required_cols = ['date', 'datetime', 'bank', 'currency', 'buy', 'sell']
        if not set(required_cols).issubset(df.columns):
            _logger.warning("Currency rates DataFrame missing required columns. Returning empty DataFrame.")
            return pd.DataFrame()
        df = df[required_cols]
        return df
    else:
...
def write_rates_to_sqlite(df, db_path='bank_data.db', table_name='currency_rates'):
    """
    Write a DataFrame of currency rates to a SQLite3 database.
    """
    if df.empty:
        _logger.warning("No rates to write to database.")
        return
    with sqlite3.connect(db_path) as conn:
        df.to_sql(table_name, conn, if_exists='append', index=False)
        _logger.info(f"Written {len(df)} rates to {table_name} in {db_path}")
...
if __name__ == '__main__':
    ...
    # Write rates to SQLite
    write_rates_to_sqlite(df)
    print(df.head())